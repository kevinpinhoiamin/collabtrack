...
public class MensagemSQL extends BaseSQL implements MensagemDAO{
    ...
    public  List<Chat> buscaPorMonitorado(Long id, boolean principal){
        ...
        try{
            ...
            if(!principal){
                String[] where = new String[]{String.valueOf(id)};
                cursor = db.query("mensagem", new String[] { ColabTrackApplication.SQL_ROW_ID_COLUMN, "*" }, "id_monitorado=? AND tipo NOT IN(2,3)", where, null, null, null, null);
            }else{
                cursor = db.query("mensagem", new String[] { ColabTrackApplication.SQL_ROW_ID_COLUMN, "*" }, "id_monitorado=?", new String[]{String.valueOf(id)}, null, null, null, null);
            }
    ...
    public Mensagem find(Long id) {
        SQLiteDatabase db = getWritableDatabase();
        try{
            Cursor c = db.query("mensagem", new String[] { ColabTrackApplication.SQL_ROW_ID_COLUMN, "*" }, "id=?", new String[]{String.valueOf(id)}, null, null, null, null);
    ...
    public Mensagem findUltimaMensagem(Long idMonitorado) {
        SQLiteDatabase db = getWritableDatabase();
        try{
            Cursor c = db.query("mensagem", new String[] { ColabTrackApplication.SQL_ROW_ID_COLUMN, "*" }, "id_monitorado=?", new String[]{String.valueOf(idMonitorado)}, null, null, "id DESC", "1");
            return parseObject(c);
        }finally {
            db.close();
        }
