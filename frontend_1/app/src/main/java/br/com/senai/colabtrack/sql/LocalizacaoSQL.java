...
public class LocalizacaoSQL extends BaseSQL implements LocalizacaoDAO{
    ...
    public List<Localizacao> findByMonitorado(long idMonitorado) {
        SQLiteDatabase db = getWritableDatabase();
        try{
            Cursor c = db.query("localizacao", new String[] { ColabTrackApplication.SQL_ROW_ID_COLUMN, "*" }, "id_monitorado=?", new String[]{String.valueOf(idMonitorado)}, null, null, null, null);
            return parseList(c);
        }finally {
            db.close();
        }
