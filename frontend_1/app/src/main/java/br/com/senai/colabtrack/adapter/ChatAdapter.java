...
import br.com.senai.colabtrack.service.DataService;
...
public class ChatAdapter extends BaseAdapter{
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(c.getTipo() == Chat.TIPO_AUDIO){
            Drawable img = ColabTrackApplication.getContext().getResources().getDrawable( R.drawable.ic_play_audio);
            img.setBounds( 0, 0, 70, 70 );
            txvMensagem.setCompoundDrawables(img, null, null, null);

            File file = new File(Environment.getExternalStorageDirectory() +
                    File.separator + "ColabTrack" + File.separator + idMonitorado+ File.separator + c.getId() +".mp3");
            ...
            File file = new File(Environment.getExternalStorageDirectory() +
                    File.separator + "ColabTrack" + File.separator + idMonitorado + File.separator + c.getIdResposta() + ".mp3");
            ...
            if(c.getTipo() == Chat.TIPO_AREA_SEGURA || (c.getTipo() == Chat.TIPO_STATUS && c.getMensagem().equals(ctx.getString(R.string.perigo)) )){
                if(c.getMensagem().equals(ctx.getString(R.string.perigo))){
                    img = ColabTrackApplication.getContext().getResources().getDrawable(R.drawable.ic_phone_chat);
                }else{
                    img = ColabTrackApplication.getContext().getResources().getDrawable(R.drawable.ic_localization);
                }
        ...
        v.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(itemChat.getTipo() == Chat.TIPO_AUDIO){
                    ...
                    File file = new File(Environment.getExternalStorageDirectory() +
                            File.separator + "ColabTrack" + File.separator + idMonitorado+ File.separator + itemChat.getId() +".mp3");
                    ...
                    if(file.exists()){
                        ...
                        Toast.makeText(ColabTrackApplication.getContext(), "Áudio não encontrado. Este áudio foi excluído.", Toast.LENGTH_LONG).show();
                    }
                }
            }
        });
